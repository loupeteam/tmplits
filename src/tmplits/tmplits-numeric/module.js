...
export function TmplitNumeric(context, args) {
    ...
    let field = '' 
    if (type === 'output'){
        field = `<div class='form-control webhmi-num-value ${classList.join(' ')}' ${dataVarName ? 'data-var-name="' + dataVarName + '"' : '' } ${attr}></div>`
    } else if (type === 'input'){
        field = `<input class='form-control webhmi-num-value ${classList.join(' ')}' ${dataVarName ? 'data-var-name="' + dataVarName + '"' : '' } ${attr}/>`
    }
